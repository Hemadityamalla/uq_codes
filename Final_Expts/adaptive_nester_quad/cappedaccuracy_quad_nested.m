...
%pp = griddedInterpolant(sort(nodes), f(sort(nodes)),'linear'); %Function evaluation here
[x,w] = fdaqr([N,D], f(nodes), L, nodes);
...
%     end
    ...
    while k_level <= level
        ...
        %Constructing the piecewise linear interpolant
        %pp = griddedInterpolant(sort(nodes), f(sort(nodes)),'linear'); %Function evaluation here
        [x,w] = fdaqr([degreeInt, length(x)], f(nodes), L, nodes);
%         %-----------Constructing the starting QR-----------
%         %Initialize quad rule
%         xstart = L(1:degreeInt);
%         wstart = ones(degreeInt,1)/(degreeInt);
